    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def build_self_attention(self, embed_dim, cfg):
        return MultiheadAttention(
            ...
            num_expert=cfg.num_expert,
            head_dim=cfg.head_dim,
            cvloss=cfg.cvloss,
            switchloss=cfg.switchloss,
            zloss=cfg.zloss,
            sample_topk=cfg.sample_topk,
            noisy_gating=True if cfg.gating.find('noisy') >=0 else False,
        )
    ...
    def forward(
        ...
        x, _, self_aux_loss = self.self_attn(
            query=x,
            key=x,
            value=x,
        ...
        # x = self.fc2(x)
        ffd_output = self.ffd(x)
        if isinstance(ffd_output, tuple):
            x, ffd_aux_loss = ffd_output
        else:
            x = ffd_output
            ffd_aux_loss = 0
        ...
        if self.return_fc and not torch.jit.is_scripting():
            return x, fc_result, self_aux_loss + ffd_aux_loss
        return x, None, self_aux_loss + ffd_aux_loss
...
class TransformerDecoderLayerBase(nn.Module):
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def __init__(
        ...
        act_list = [ModuleWrapper(self.activation_fn),
    ...
    def forward(
        ...
        x, attn, self_aux_loss = self.self_attn(
            query=x,
            key=y,
            value=y,
        ...
        if self.encoder_attn is not None and encoder_out is not None:
            ...
            x, attn, cross_aux_loss = self.encoder_attn(
                query=x,
                key=encoder_out,
                value=encoder_out,
                query_padding_mask=self_attn_padding_mask,
        ...
        if self.onnx_trace and incremental_state is not None:
            ...
            return x, attn, self_attn_state, self_aux_loss+cross_aux_loss+ffd_aux_loss
        return x, attn, None, self_aux_loss+cross_aux_loss+ffd_aux_loss
